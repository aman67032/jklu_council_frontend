...
vec3 getLineColor(float t, vec3 baseColor) {
  ...
  return isLightMode ? gradientColor * 0.9 : gradientColor * 0.5;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 col = isLightMode ? vec3(0.98, 0.977, 0.965) : vec3(0.0);
  vec3 b = lineGradientCount > 0 ? (isLightMode ? vec3(0.98, 0.977, 0.965) : vec3(0.0)) : background_color(baseUv);
  ...
  if (enableBottom) {
    for (int i = 0; i < bottomLineCount; ++i) {
      ...
      if (isLightMode) {
        // Balanced visibility for light mode
        col = mix(col, lineCol, clamp(waveVal * 1.6, 0.0, 1.0));
      } else {
        col += lineCol * waveVal;
      }
    }
  ...
  if (enableMiddle) {
    for (int i = 0; i < middleLineCount; ++i) {
      ...
      if (isLightMode) {
        // Balanced visibility for light mode
        col = mix(col, lineCol, clamp(waveVal * 1.4, 0.0, 1.0));
      } else {
        col += lineCol * waveVal;
      }
    }
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    if (interactive) {
      ...
      // Fallback for older browsers if needed, though pointer events are modern
